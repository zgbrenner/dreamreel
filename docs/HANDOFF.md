...
| — | **Memory system — dream recurrence (RAM++)** | ✅ Phase 1 code merged (`embed/entities.py` RAM++ tags → `entities[]`; `dream/memory.ts` DreamMemory decaying entity weights; `dreamwalker` recurrence bias; `conductor` per-beat observe). Motifs recur, bounded + relaxing + deterministic. ⬜ RAM++ run + reship pending. ⬜ **Phase 2:** Grounding DINO + SAM 2 for literal segmented-entity reuse (all Apache-2.0, code+weights verified) |
