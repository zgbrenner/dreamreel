...
| — | **Memory system — dream recurrence (RAM++)** | ✅ Phase 1 code merged + live-in-app (`dream/memory.ts` DreamMemory; `dreamwalker` recurrence bias; `conductor` per-beat observe — **dormant until entities ship**, graceful no-op). RAM++ tagger (`embed/entities.py`) works on transformers 5.x (compat shim in `_make_ram`). ⬜ **Entity reship BLOCKED here:** the ~3 GB RAM++ swin-large checkpoint stalls on this throttled HF connection (same as so400m — got 255 MB then froze). Run `pip install -e .[entities]` then `python -m embed.entities --upload` from a machine with bandwidth (or set `HF_TOKEN` to avoid the unauthenticated throttle); no smaller RAM++ variant exists. ⬜ **Phase 2:** Grounding DINO + SAM 2 for literal segmented-entity reuse (all Apache-2.0, code+weights verified) |
